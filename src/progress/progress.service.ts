...
export class ProgressService {
  ...
  async create(currUser: User, createProgressDto: CreateProgressDto) {
    try {
      const existingCourse = await this.courseRepository.findOne({ where: { id: createProgressDto.course } });
      if (!existingCourse) {
        throw new NotFoundException(ERRORS.ERROR_COURSE_NOT_FOUND)
      }
      const existingModule = await this.moduleRepository.findOne({ where: { id: createProgressDto.module } });
      if (!existingModule) {
  ...
  async remove(currUser: User, id: string) {
    try {
      ...
      const progress = await this.progressRepository.findOne({ where: { id: id, student: { id: currUser.id } } });
      if (!progress) {
