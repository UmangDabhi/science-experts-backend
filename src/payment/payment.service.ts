...
export class PaymentService {
  ...
  async createOrder(currUser: User, createPaymentDto: CreatePaymentDto) {
    ...
    if (createPaymentDto.type === PURCHASE_OF_TYPE.COURSE) {
      const existingCourse = await this.courseRepository.findOne({
        where: { id: createPaymentDto.item_id },
      });
    ...
    const totalExpertCoins =
      await this.userBalanceService.getAllCoins(currUser);
    ...
    if (remainingAmount === 0) {
      const coinsSuccessfullyDeducted =
      ...
      await this.afterSuccessfulPurchase(
        currUser,
        createPaymentDto.type,
        createPaymentDto.item_id,
      );
      await this.paymentRepository.save({
        status: 'SUCCESS',
        type: createPaymentDto.type as PURCHASE_OF_TYPE,
        itemId: createPaymentDto.item_id,
        user: currUser,
        orderId: `COIN_ORDER_${Date.now()}`,
        paymentId: `COIN_INTERNAL_${Date.now()}`,
        amount: 0,
      });
      return {
        success: true,
        collect_payment: false,
        message: 'Fully covered by coins',
      };
    }
  ...
  async verifyPayment(currUser: User, verifyPaymentDto: VerifyPaymentDto) {
    try {
      ...
      if (verifyPaymentDto.coins_used && verifyPaymentDto.coins_used > 0) {
        const coinsSuccessfullyDeducted =
          await this.userBalanceService.deductCoins(
            currUser,
            verifyPaymentDto.coins_used,
          );
        if (!coinsSuccessfullyDeducted) {
          return {
            success: false,
      ...
      if (expectedSignature === razorpay_signature) {
        await this.afterSuccessfulPurchase(
          currUser,
          verifyPaymentDto.type,
          verifyPaymentDto.item_id,
        );
        await this.paymentRepository.save({
          status: 'SUCCESS',
          itemId: verifyPaymentDto.item_id,
          user: currUser,
          type: verifyPaymentDto.type as PURCHASE_OF_TYPE,
          orderId: razorpay_order_id,
          paymentId: razorpay_payment_id,
          amount: Number(verifyPaymentDto.amount) / 100,
        });
  ...
  private async afterSuccessfulPurchase(
    currUser: User,
    type: string,
    item_id: string,
  ) {
    if (type === PURCHASE_OF_TYPE.COURSE) {
      ...
      await this.materialPurchaseService.create(currUser, {
        material: item_id,
      });
